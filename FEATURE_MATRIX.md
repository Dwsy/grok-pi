...
| Pi subagents | Native+Adapted | Built-in `pi-grok-subagents` extension owns a Pi child `AgentSession`; its versioned bridge projects to native `SubagentBlock`, Tasks Pane, child `AgentView`, and `x.ai/subagent/cancel`. `/subagents` displays the built-in profiles and writes only product-isolated project/global Markdown overrides (including restore/remove override), uses Pi runtime tool catalog groups and up to three models, and opens the existing Pi resource manager in selection mode for extensions/skills without mutating Pi resource settings. `/subagent-message` and `send_message_to_subagent` deliver follow-up or steer text to a running child. Model-driven end-to-end acceptance remains pending. |
