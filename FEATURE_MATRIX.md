...
| Mid-turn send now | Adapted | Composer send-now → Pi `steer`. A pending adapter-owned row is atomically removed from the isolated queue and promoted as `steer`; edit-then-send uses the row version. Rows discovered only in Pi's external queue remain read-only because stock RPC cannot remove them atomically. |
| Follow-up queue | Adapted | Active-turn client prompts and extension `sendUserMessage(..., { deliverAs: "followUp" })` calls stay in an adapter-owned isolated queue until dispatch. Promoted rows re-enter Pi with RPC source; `queue_update` messages that bypass interception are mirrored in a separate external lane. |
...
| Pi subagents | Native+Adapted | F2 `[ui].pi_subagents` is default on and restart-required. When enabled, the built-in `pi-grok-subagents` extension owns a Pi child `AgentSession`; its versioned bridge projects to native `SubagentBlock`, Tasks Pane, child `AgentView`, and `x.ai/subagent/cancel`. `/subagents` displays the built-in profiles and writes only product-isolated project/global Markdown overrides (including restore/remove override), uses Pi runtime tool catalog groups and up to three models, and opens the existing Pi resource manager in selection mode for extensions/skills without mutating Pi resource settings. `/subagent-message` and `send_message_to_subagent` deliver follow-up or steer text to a running child. When disabled, the bridge is omitted and inherited activation metadata is cleared. Model-driven end-to-end acceptance remains pending. |
...
| Session code review (`/review-session`, `/review-message`) | Native | PSM code-review mechanism → native Pager modal: left file/path list (flat or tree, without per-row diffstat), right BlockViewer edit preview, and a lower Q&A panel with streamed answers plus a separated bottom input (default filter = changes only). **Opt/Alt+Shift+R** opens the session review; F2 `review_file_tree` default **off** (persisted); modal `t` toggles tree (cwd-relative, compact Java packages with `.`). `/review-message` reuses jump ListOverlay. No Pi custom UI. |
